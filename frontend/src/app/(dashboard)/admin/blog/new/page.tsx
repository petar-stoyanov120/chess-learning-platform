...
export default function AdminNewBlogPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const res = await api.post<{ data: { id: number; slug: string } }>('/blog', {
        title, content, excerpt,
        tagIds: selectedTags,
        variations: variations.filter((v) => v.name.trim() && v.notation.trim()),
      });
      const post = (res as { data: { id: number; slug: string } }).data;
      if (publish) {
        await api.patch(`/blog/${post.id}/approve`, {});
      }
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-chess-dark">New Blog Post</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        {/* Variations (optional chess positions) */}
