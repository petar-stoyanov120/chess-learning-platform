...
import { Category, DifficultyLevel, Lesson } from '@/lib/types';
...
export default function AdminEditLessonPage() {
  ...
  async function handlePublish() {
    setSaving(true);
    try {
      await api.patch(`/lessons/${lessonId}/status`, { status: 'ready' });
      router.push('/admin/lessons');
    } catch (err) {
      setError(err instanceof Error ? err.message : 'Failed to mark lesson as ready.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <span className="text-sm text-gray-400 capitalize">({statusName})</span>
      </div>
