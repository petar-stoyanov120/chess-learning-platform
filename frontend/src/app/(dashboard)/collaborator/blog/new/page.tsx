...
export default function CollaboratorNewBlogPage() {
  ...
  useEffect(() => {
    api.get<{ data: Tag[] }>('/tags').then((res) => setTags((res as { data: Tag[] }).data));
  }, []);
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError('');
    try {
      const res = await api.post<{ data: { id: number } }>('/blog', {
        title, content, excerpt,
        tagIds: selectedTags,
        variations: variations.filter(v => v.name.trim() && v.notation.trim()),
      });
      if (submitForReview) {
        await api.patch(`/blog/${(res as { data: { id: number } }).data.id}/submit`, {});
      }
  ...
  return (
    <div className="max-w-4xl">
      <div className="flex items-center gap-4 mb-6">
        ...
        <h1 className="text-2xl font-bold text-chess-dark">Write a Blog Post</h1>
      </div>
      <p className="text-gray-500 mb-6 text-sm bg-blue-50 border border-blue-100 rounded-lg px-4 py-3">
        💡 Your post will be reviewed by an admin before being published publicly.
      </p>
        ...
        <div className="card p-6 space-y-4">
          <div>
            ...
            <input type="text" value={title} onChange={(e) => setTitle(e.target.value)} required
              ...
              placeholder="e.g. How to Study Chess Openings Effectively" />
          </div>
          <div>
