...
export default function EditPuzzlePage() {
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        setMaxMoves(p.maxMoves != null ? String(p.maxMoves) : '');
        setFen(p.fen);
        setSideToMove(p.sideToMove as 'white' | 'black');
        if (p.dueDate) {
          setDueDate(new Date(p.dueDate).toISOString().slice(0, 16));
        }
      } catch {
  ...
  const hasSubmissions = (puzzle?._count?.submissions ?? 0) > 0;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setSaving(true);
    try {
      await api.patch(`/classrooms/${classroomId}/puzzles/${puzzleId}`, {
        title: title.trim(),
        description: description.trim() || null,
        solution: solution.trim() || null,
        maxMoves: maxMoves ? parseInt(maxMoves) : null,
        fen: fen.trim(),
        sideToMove,
        dueDate: dueDate ? new Date(dueDate).toISOString() : null,
      });
      success('Puzzle updated!');
      router.push(`/collaborator/classrooms/${classroomId}?tab=puzzles`);
    } catch (err: any) {
      error(err?.response?.data?.message || 'Failed to update puzzle.');
    } finally {
      setSaving(false);
    }
  }
  ...
  return (
    <div className="max-w-3xl">
      <div className="flex items-center gap-3 mb-6">
        ...
        <h1 className="text-xl font-bold text-chess-dark dark:text-gray-100">Edit Puzzle</h1>
      </div>
      ...
      {hasSubmissions && (
        <div className="mb-4 bg-amber-50 dark:bg-amber-900/20 border border-amber-200 dark:border-amber-800 rounded-xl p-4 text-sm text-amber-800 dark:text-amber-200">
          <strong>Note:</strong> This puzzle has {puzzle?._count?.submissions} submission(s). The position and title are locked — only the due date, solution, and move limit can be updated.
        </div>
      )}
        ...
        <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-2xl p-6 space-y-5">
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-chess-gold disabled:bg-gray-50 dark:disabled:bg-gray-700/50 disabled:text-gray-400"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              disabled={hasSubmissions}
              className="w-full border border-gray-300 dark:border-gray-600 dark:bg-gray-700 dark:text-gray-100 rounded-lg px-3 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-chess-gold resize-none disabled:bg-gray-50 dark:disabled:bg-gray-700/50 disabled:text-gray-400"
            />
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1.5">Move Limit (optional)</label>
              <input
                type="number"
                value={maxMoves}
